...
| networkPolicy.cilium.ingress | list | `[ {toPorts: [{ports: [port: http]}]} ]` | Cilium network policy ingress https://docs.cilium.io/en/stable/security/policy/ |
...
| networkPolicy.ingress  | list | `[ {ports: [ {protocol: TCP, port: http} ]` |  | Kubernetes ingress spec https://kubernetes.io/docs/concepts/services-networking/network-policies/ |
...
| priorityClassName | string | `""` | Priority class name for the node-red pod. default: "" |
...
| resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"128Mi"}}` | CPU/Memory resource requests/limits |
...
| startupProbe | object | `{"httpGet":{"path":"/","port":"http"}}` | Startup probe for the Deployment |
